...
export function Collections() {
  ...
  const fetchTokens = ({ pageParam = 0 }) =>
    getTokens(owner, keyword, sortBy, isDesc ? "desc" : "asc", pageParam);
    ...
    <section className="container container-xl">
      <section className="collections-topbar">
        ...
        <section className="stats">
          <StatCard
            label="ASSETS"
            value={stats?.stats?.tokens?.toLocaleString() ?? "-"}
          />
          <StatCard
            label="HOLDERS"
            value={stats?.stats?.owners?.toLocaleString() ?? "-"}
          />
          <StatCard
            label="VOLUME"
            value={stats?.stats?.volume?.daily?.toLocaleString() ?? "-"}
          />
          <StatCard
            label="FLOOR PRICE"
